...
class skylight extends CI_Controller {
       ...
       // var_dump($this->output_type);
        ...
        switch ($this->output_type) {
            ...
            case 'xml':
                ...
                break;
            case 'cerif':
               if (file_exists('./application/views/formats/cerif/' . $view . '.php')) {
                   // var_dump($this->output_type);
                    $this->load->view('formats/cerif/' . $view, $data);
                }
            default:
                // Get the theme
    ...
    function _conneg($in) {
            ...
            return substr($in, 0, strlen($in) - 4);
        } else if ($this->_endswith($in, '.cerif')) {
            $this->output_type = 'cerif';
            return substr($in, 0, strlen($in) - 6);
        }
