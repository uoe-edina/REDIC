...
class Solr_client_dspace_181 {
    ...
    var $date_field = 'dc.date.created_dt';
    var $delimiter      = '';
    var $thumbnail_field = '';
    var $bitstream_field = '';
   ...
   /*     foreach($this->configured_date_filters as $filter_name => $filter) {
     ...
     /*       foreach($range as $var1 => $var2) {
                if(preg_match("/fq/",$var1)) {
                    $url .= '&facet.query='.$var2;
                }
                foreach($var2 as $num => $yearRange) {
                    var_dump($yearRange);
                }
            } */
        ...
        //print_r('facets ' . $url);
//       $url .= '&facet.query=dateIssued.year:[2010+TO+2012]';
    //var_dump($url);
    ...
    function browseTerms($field = 'Subject', $rows = 10, $offset = 0, $prefix = '') {
        ...
        $url .= '&fq='.$this->container_field.':'.$this->container;
        $url .= '&fq=search.resourcetype:2&rows=0&facet.mincount=1';
       // var_dump($this->configured_filters);
        //var_dump($this->configured_date_filters["Date"]);
         if (preg_match("/Date/", $field)) {
            $url .= '&facet=true&facet.sort=index&facet.field='.$this->configured_date_filters[$field].'&facet.limit='.$rows.'&facet.offset='.$offset;
        }
        else {
       ...
       // var_dump($url);
      // var_dump(preg_match("/Date/", $field));
        $solr_xml = file_get_contents($url);
        ...
        if (preg_match("/Date/", $field)) {
            $facet_xml = $search_xml->xpath("//lst[@name='facet_fields']/lst[@name='".$this->configured_date_filters[$field]."']/int");
            // var_dump($search_xml);
        }
        else {
        ...
        // Build facets from solr response
        foreach ($facet_xml as $facet_term) {
               // var_dump($facet_term->attributes());
                //var_dump($field);
                if(preg_match("/Type/", $field)  || preg_match("/Date/", $field)) {
                    //var_dump($facet_term->attributes());
                    $names = preg_split('/\|\|\|/',$facet_term->attributes());
                    $term['name'] = urlencode($facet_term->attributes());
                    $term['display_name'] = urlencode($names[0]);
                    $term['norm_name'] = urlencode($names[0]);
                    $term['count'] = $facet_term;
                    $terms[] = $term;
                } 
    ...
    function getLowerBound($field, $q, $fq) {
        ...
        $value = 2010; // Stupid default to catch problems
       // var_dump($field);
        $url = $this->base_url . "select/?q=" . $this->solrEscape($q);
        if(count($fq) > 0) {
            foreach($fq as $value)
    ...
    function getUpperBound($field, $q, $fq) {
        ...
        $value = 2020; // Stupid default to catch problems
